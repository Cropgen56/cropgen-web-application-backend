...
export const getDashboardAnalytics = async (req, res) => {
  try {
    ...
    const activeUsers = await User.countDocuments({ lastActiveAt: { $gte: start, $lte: end } }); 
    const usersByRole = await User.aggregate([
      { $group: { _id: '$role', count: { $sum: 1 } } },
      { $project: { role: '$_id', count: 1, _id: 0 } },
    ]);

    // New Users Details (sorted newest first, select relevant fields)
    const newUsersDetails = await User.find({ createdAt: { $gte: start, $lte: end } })
      .sort({ createdAt: -1 })
      .select('firstName lastName avatar email phone role organization terms lastLoginAt lastActiveAt createdAt updatedAt');

    // FarmField Metrics
    const totalFields = await FarmField.countDocuments();
    const newFields = await FarmField.countDocuments({ createdAt: { $gte: start, $lte: end } }); 
    ...
    // New Fields Details (sorted newest first, populate user info)
    const newFieldsDetails = await FarmField.find({ createdAt: { $gte: start, $lte: end } })
      .sort({ createdAt: -1 })
      .populate('user', 'firstName lastName email phone role')
      .select('field user fieldName cropName variety sowingDate acre typeOfIrrigation typeOfFarming createdAt updatedAt');
